...
function testCompositeCreated() {
    ...
    if ! assertCurl 200 "curl -k https://$HOST:$PORT/product-composite/$PROD_ID_REVS_RECS -s"
    then
        echo -n "FAIL"
        return 1
    fi
...
function recreateComposite() {
  ...
  assertCurl 202 "curl -X DELETE -k https://$HOST:$PORT/product-composite/${productId} -s"
...
assertCurl 404 "curl -k https://$HOST:$PORT/product-composite/$PROD_ID_NOT_FOUND -s"
...
assertCurl 200 "curl -k https://$HOST:$PORT/product-composite/$PROD_ID_NO_RECS -s"
...
assertCurl 200 "curl -k https://$HOST:$PORT/product-composite/$PROD_ID_NO_REVS -s"
assertEqual $PROD_ID_NO_REVS "$(echo "$RESPONSE" | jq .productId)"
assertEqual 3 "$(echo "$RESPONSE" | jq ".recommendations | length")"
assertEqual 0 "$(echo "$RESPONSE" | jq ".reviews | length")"

# Verify that a 422 (Unprocessable Entity) error is returned for a productId that is out of range (-1)
assertCurl 422 "curl -k https://$HOST:$PORT/product-composite/-1 -s"
assertEqual "\"Invalid productId: -1\"" "$(echo "$RESPONSE" | jq .message)"

# Verify that a 400 (Bad Request) error error is returned for a productId that is not a number, i.e. invalid format
assertCurl 400 "curl -k https://$HOST:$PORT/product-composite/invalidProductId -s"
